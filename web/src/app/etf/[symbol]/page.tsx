...
export default async function ETFDetailPage(props: ETFDetailProps) {
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
                    <div className="flex items-start justify-between">
                        <div>
                            ...
                            <p className="text-gray-600">{etf.underlying_asset || 'ETF'}</p>
                            {etf.name && etf.name !== symbol && (
                                <p className="text-sm text-gray-500 mt-1">{etf.name}</p>
                            )}
                        </div>
                        <div className="text-right">
                            <div className="text-sm text-gray-500 mb-1">Current Price</div>
                            <div className="text-3xl font-bold text-gray-900">
                                {latestPrice ? `₹${latestPrice.close_price.toFixed(2)}` : '-'}
                            </div>
                            {latestPrice && (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                        <h2 className="text-xl font-bold text-gray-900 mb-4">ETF Information</h2>
                        <div className="space-y-3">
                            ...
                            <div className="flex justify-between">
                                ...
                                <span className="font-medium text-gray-900">{etf.underlying_asset || '-'}</span>
                            </div>
                            {etf.nav && (
                                <div className="flex justify-between">
                                    <span className="text-gray-600">NAV</span>
                                    <span className="font-medium text-gray-900">₹{etf.nav.toFixed(2)}</span>
                                </div>
                            )}
                            {perf?.daily_volume && (
                                <div className="flex justify-between">
