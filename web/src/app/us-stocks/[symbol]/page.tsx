...
interface USDetailProps {
    params: Promise<{ symbol: string }>
}

export default async function USDetailPage(props: USDetailProps) {
    const params = await props.params
    const ticker = params.symbol

    const stock = await prisma.us_stocks.findUnique({
        where: { ticker: ticker },
        include: {
            us_daily_prices: {
                orderBy: { date: 'asc' },
    ...
    const latestPrice = stock.us_daily_prices[stock.us_daily_prices.length - 1]
    const chartData = stock.us_daily_prices.map(p => ({
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
                    <div className="flex items-start justify-between">
                        <div>
                            ...
                            <p className="text-gray-600">{stock.name || 'US Stock'}</p>
                            <p className="text-xs text-gray-400 mt-1">Russell 1000{stock.sector ? ` · ${stock.sector}` : ''}</p>
                        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
                        <h2 className="text-xl font-bold text-gray-900 mb-4">US Stock Information</h2>
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {[...stock.us_daily_prices].reverse().slice(0, 20).map((price) => (
