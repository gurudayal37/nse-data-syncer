...
OUTPUT_PATH   = os.path.join(base_dir, 'web', 'src', 'data', 'backtest_results_us_momentum_10pct_stop.json')
N_STOCKS      = 15
...
def process_period(rebalance_date, next_date, master_df, stock_map, benchmark_df, label=None):
    ...
    for sid in ids:
        ...
        for _, daily in rows.iterrows():
            if start_p and (daily['close_price'] - start_p) / start_p <= STOP_LOSS_PCT:
                stop_hit = True
                break
        if stop_hit:
            r = STOP_LOSS_PCT
            ...
            r = 0.0
        rets.append(r)
        holdings.append({'symbol': stock_map.get(sid,'?'), 'return': round(r*100,2),
                         'score': round(score_map.get(sid,0),2), 'stop_triggered': stop_hit})
...
def run_backtest():
    print("=== US Momentum Strategy (3M+6M+1Y, 10% Stop) ===")
    db = DatabaseManager()
    session = db.Session()
    try:
        # Benchmark
